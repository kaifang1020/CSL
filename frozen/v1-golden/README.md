...
| Greg | `data/GREG_v3/01920.jpg` | `4a13aee2b5b8b7d09ec0ac1d68d6d089` | 正脸 0.992、**唇抿合**(外唇厚 0.160)、睁眼 0.296、神情疲惫不带笑 |
...
- **Greg 素材的黑卡区间与 Candice 同类**:`GREG_v3.mp4`(31MB, md5 `2360870936fd8251ce7e2786a8588102`,
  1920x1080 @ 29.97fps, 87.6s)首尾各有约 8 秒黑底卡片 —— 0–8.04s 标题卡「Greg」、
  79.55–87.55s「Now it's your turn to speak.」。**预处理不重采样**(2625 帧 1:1 对应源 fps),
  所以安全帧区间是 **242–2384**,挑参考帧必须带 `--start 242 --end 2384`。
  注意黑底白字帧的均值只有 3–4,靠「非纯色」判断会放过它们。
- **`pick_ref.py` 的两个指标在老年/戴镜脸上失效**:内唇缝 `k[62]-k[66]` 会饱和到 0.0000
  (742 帧里一大片并列),分不出「刚碰上」和「抿紧」——应改用外唇厚 `k[51]-k[57]`;
  eye-aspect-ratio 被眼镜干扰,闭着眼的 01928 得分(0.335)反而高于睁眼的 01922(0.316)。

- **参考帧 md5 不能跨机器复现**(2026-08-20 实测)。`preprocess_user_video.py` 用**全片平均人脸框**
  裁剪,而 s3fd 检测是 GPU 浮点运算 —— 换架构后 2625 帧平均下来的框会偏不到 1px,取整即差 1px。
  Greg 的 `01920.jpg` 在 4090 上是 `4a13aee2…`,在 A100 上是 `97fec6d5…`,
  互相关对齐后平均差 3.152 → 0.681,**位移 dy=-1 dx=+1,画面实质相同**。
  ⇒ md5 只能作为**同机器内**的一致性校验;跨机器要认的是**帧号 + 源视频 md5 + 挑帧区间**。
